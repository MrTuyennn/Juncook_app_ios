            ...
            ZStack(alignment: .top){
                VStack(alignment: .leading,spacing: 20){
                    ...
                    VStack(alignment: .leading,spacing: 20){
                        ...
                        HStack {
                            Text("\(ship.locationName)").font(.custom("Poppin-Regular", size: 16)).foregroundColor(.black)
                            Spacer()
                            Text("\(ship.price)").font(.custom("Poppin-Regular", size: 16)).foregroundColor(.black)
                        }
                        
                        Text("Description").font(.custom("Poppin-Regular", size: 16)).foregroundColor(.black)
                        
                        ScrollView(.vertical,showsIndicators: false){
                            Text(ship.description).font(.custom("Poppin-Light", size: 16)).foregroundColor(.gray)
                        }
                    }.padding(.horizontal)
                ...
                .navigationTitle("\(ship.displayName),\(ship.locationName)")
...
#Preview {
    ShipDetailView(ship: Ship.celebrityinfinity)
}
